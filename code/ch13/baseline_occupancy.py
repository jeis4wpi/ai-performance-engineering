"""baseline_occupancy.py - Low occupancy baseline in training context.

Demonstrates low occupancy causing GPU underutilization during training.
Occupancy: This baseline has low occupancy (too few threads per SM).
Causes GPU underutilization and poor training performance.
...
class BaselineOccupancyBenchmark(Benchmark):
    ...
    def setup(self) -> None:
        ...
        self.input = torch.randn(4, 1024, device=self.device)  # Small batch
    ...
    def benchmark_fn(self) -> None:
        ...
        try:
            ...
            # - Poor training performance due to limited parallelism
        finally:
