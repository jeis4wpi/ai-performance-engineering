// baseline_cuda_graphs.cu -- Separate kernel launches (baseline).
...
int main() {
  ...
  printf("Baseline CUDA Graphs (separate launches) on %s (CC %d.%d)\n", prop.name, prop.major, prop.minor);
  ...
  printf("Baseline (separate launches): %.3f ms (%.3f ms/iter)\n", ms, ms / ITER);
