"""optimized_nvlink.py - Optimized NVLink for high-speed GPU communication in occupancy/warp divergence context.
        ...
        if num_gpus < 2:
            ...
            if torch.cuda.can_device_access_peer(0, 1):
                try:
                    ...
                    # Use torch._C._cuda_enablePeerAccess for PyTorch compatibility
                    torch.cuda.set_device(0)
                    if hasattr(torch._C, '_cuda_enablePeerAccess'):
                        torch._C._cuda_enablePeerAccess(1)
                    else:
                        # Fallback: use context manager if available
                    ...
                    # Ensure all peer access operations complete before proceeding
                    torch.cuda.set_device(0)
