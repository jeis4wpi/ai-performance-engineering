"""stream_ordered_cuda_graphs.py - Concurrent kernel execution with streams (optimized).
...
class OptimizedStreamOrderedCudaGraphsBenchmark(Benchmark):
    """Stream-ordered CUDA graphs - kernels overlap.
    
    Note: For warp specialization examples, see stream_ordered_cuda_graphs_warp_specialized.cu
    ...
    def benchmark_fn(self) -> None:
        ...
        torch.cuda.nvtx.range_push("stream_ordered_cuda_graphs_concurrent")
        try:
...
def get_benchmark() -> Benchmark:
    ...
    return OptimizedStreamOrderedCudaGraphsBenchmark()
...
if __name__ == '__main__':
    ...
    print(f"\nOptimized Streams: {result.mean_ms:.3f} ms")
