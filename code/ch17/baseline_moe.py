"""baseline_moe.py - Baseline dense model (no MoE) in inference/profiling context.

Demonstrates dense model where all parameters are active.
MoE: This baseline does not use Mixture of Experts.
All parameters process every input, causing inefficient computation.
...
class BaselineMoeBenchmark(Benchmark):
    ...
    MoE: This baseline does not use Mixture of Experts.
    All parameters are active for every input, causing inefficient computation.
    """
    ...
    def setup(self) -> None:
        ...
        # MoE (Mixture of Experts) uses sparse activation - only selected experts process input
